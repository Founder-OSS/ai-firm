...
async function sendToAgent(msg: string): Promise<string> {
  try {
    const res = await fetch("http://localhost:8080/api/agent/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: msg }) });
    const data = await res.json();
    return data.reply;
  } catch { return "Backend offline. Run: cd ~/ai-firm/backend && cargo run"; }
}
export default function Home() {
  const [messages, setMessages] = useState<Message[]>([{ role: "agent", text: "ai-firm agent online. The Way to Rust begins here.", ts: new Date().toLocaleTimeString() }]);
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  const [status, setStatus] = useState<"checking"|"online"|"offline">("checking");
  const bottomRef = useRef<HTMLDivElement>(null);
  useEffect(() => { fetch("http://localhost:8080/api/health").then(r=>r.json()).then(()=>setStatus("online")).catch(()=>setStatus("offline")); }, []);
  useEffect(() => { bottomRef.current?.scrollIntoView({ behavior: "smooth" }); }, [messages]);
  const send = async () => {
    if (!input.trim() || loading) return;
    const u: Message = { role: "user", text: input, ts: new Date().toLocaleTimeString() };
    setMessages(m=>[...m,u]); setInput(""); setLoading(true);
    const reply = await sendToAgent(u.text);
    ...
    <div style={{minHeight:"100vh",background:"#0a0a0a",color:"#e8e0d0",fontFamily:"monospace",display:"flex",flexDirection:"column"}}>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{flex:1,overflowY:"auto",padding:"24px 28px",display:"flex",flexDirection:"column",gap:16}}>
          {messages.map((msg,i)=>(
            <div key={i} style={{display:"flex",flexDirection:"column",alignItems:msg.role==="user"?"flex-end":"flex-start",gap:3}}>
              ...
              <div style={{maxWidth:"70%",padding:"10px 14px",background:msg.role==="user"?"#140a04":"#0f0f0f",border:`1px solid ${msg.role==="user"?"#3a1505":"#1c1c1c"}`,fontSize:13,lineHeight:1.7,color:msg.role==="user"?"#e8c090":"#9a9288"}}>{msg.text}</div>
            </div>
          ))}
        ...
        <div style={{borderTop:"1px solid #1a1a1a",padding:"14px 28px",display:"flex",gap:10,alignItems:"center"}}>
          ...
          <button onClick={send} disabled={loading} style={{background:"#c0622f",border:"none",color:"#fff",padding:"7px 18px",fontSize:11,letterSpacing:2,cursor:"pointer",fontFamily:"monospace"}}>SEND</button>
        </div>
      </main>
    </div>
  );
